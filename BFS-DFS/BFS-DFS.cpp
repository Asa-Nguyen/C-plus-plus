...
int DayGiaTriVaoQueue(QUEUE &Q,int value){
	...
	Q.size++;
	return 1;//Day them gia tri vao QUEUE THANH CONG
...
int LayGiaTriRaKhoiQueue(QUEUE &Q,int &value){
	...
	Q.size--;
	return 1;//lay gia tri khoi Queue thanh cong
}

int KiemTraQueueRong(QUEUE Q){
	if(Q.size<=0){
		return 1;//Neu rong tra ve 1
	}
	return 0;//Neu khong rong tra ve 0
}
...
int main()
{
	...
	if(DocMaTranKe(inputfile,g)==1){
		...
		getch();
		cout<<"Bam 1 phim bat ki de bat dau duyet theo BFS...\n\n";
		getch();
		duyettheoBFS(1,4,g);
		// for(int i=0;i<g.n;i++){
		// 	// if(i==f){
